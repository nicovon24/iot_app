...
**Status:** Discussed and designed, plan approved by user in a Claude Code session (not yet run through `/paul:plan`) — **runs after Phase 11 (testing harness)**, per explicit user sequencing
...
Va después de Fase 11 (testing harness, ya reservada en `.paul/phases/11-testing-harness/CONTEXT.md` pero sin plan todavía — se agenda recién cuando el usuario confirme que el backend V1 está terminado, no antes). Este plan no incluye setup de Jest ni tests automatizados; solo los `*.check.ts` assert-based manuales que ya son el patrón del repo.
