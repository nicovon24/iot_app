...
/** Widget types whose config carries a min/max scale. Mirrors the backend schemas. Exported once
 * here rather than declared separately in both ConfigureStep.tsx and AddWidgetPanel/index.tsx. */
export const SCALE_TYPES: WidgetType[] = ['gauge', 'battery', 'rssi'];
...
export const WIDGET_REGISTRY: Record<WidgetType, WidgetTypeMeta> = {
  ...
  'multi-key-chart': {
    ...
    description: 'Two or more keys on one chart, one entity',
    icon: GitCompare,
    category: 'Charts',
    entity: 'required',
    telemetryKey: 'none',
    multiTelemetryKeys: true,
    entityKinds: ['DEVICE', 'ASSET'],
    // N entities x M keys isn't a chart anyone can read — this widget answers "how do this
    // entity's own keys relate", not a fleet-comparison question.
    supportsAllScope: false,
    supportsDataKeys: false,
    defaultLayout: { w: 4, h: 3 },
  },
  ...
  // The first widget with entity: 'none' — static text, no datasource at all. Grouped under
  // Cards rather than a new one-item category.
  label: {
    label: 'Label',
    description: 'Static text — headers, notes, instructions',
    icon: Type,
    category: 'Cards',
    entity: 'none',
    telemetryKey: 'none',
