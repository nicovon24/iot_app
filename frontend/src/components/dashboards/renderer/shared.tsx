import type { GaugeStyle } from '@/widgets';
...
export interface EntityWidgetConfig extends WidgetDatasource {
  ...
  /** Per-key unit for multi-key widgets (value-cards, timeseries-table, multi-key-chart) —
   * unlike the singular `unit` field above, one entry per telemetry key. */
  units?: Record<string, string>;
  /** Gauge, battery, rssi — dial calibration. */
  min?: number;
  max?: number;
  /** Unit/decimals for the rendered value — applies to any value-carrying widget, not just the
   * dial types (that used to be the only ones with a `unit` field). */
  unit?: string;
  decimals?: number;
  ...
  title?: string;
  action?: WidgetAction;
  /** Label widget — static text, no datasource. */
  text?: string;
  align?: 'left' | 'center';
  /** Bar chart — grouped vs stacked, meaningful only under entityScope: 'ALL'. */
  stacked?: boolean;
  /** Value tile — a mini trend line, meaningful only for SINGLE scope. */
  sparkline?: boolean;
}
