...
export type { MeasureFormat } from '@/types';

/**
 * Rounds a numeric telemetry value to at most `decimals` places (default 2) for display, and
 * appends `unit` when the value is numeric. Non-numeric values (e.g. "state": "RUNNING",
 * "alarmCode": "NONE") pass through unchanged — never get a unit suffix, never get rounded —
...
export function formatTelemetryValue(raw: string | number | undefined, opts?: MeasureFormat): string | undefined {
  ...
  const rounded = String(Number(num.toFixed(decimals)));
  return opts?.unit ? `${rounded} ${opts.unit}` : rounded;
}
